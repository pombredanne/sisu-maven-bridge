 ...
 */
package io.tesla.maven.bridge.support.model.internal;

import static io.tesla.maven.bridge.support.ArtifactRequestBuilder.request;
import static io.tesla.maven.bridge.support.RemoteRepositoryBuilder.remoteRepositories;
import static io.tesla.maven.bridge.support.RemoteRepositoryBuilder.remoteRepository;
import static java.util.Arrays.asList;

import io.tesla.maven.bridge.MavenArtifactResolver;
import io.tesla.maven.bridge.MavenModelResolver;
import io.tesla.maven.bridge.internal.ComponentSupport;
...
public abstract class MavenModelResolverSupport extends ComponentSupport implements MavenModelResolver {
  ...
  protected MavenModelResolverSupport(final MavenArtifactResolver artifactResolver, final @Nullable Provider<RepositorySystemSession> sessionProvider) {
  ...
  @Override
  public Model resolveModel(final ModelBuildingRequest request, final RepositorySystemSession session, final RemoteRepository... repositories) throws ModelBuildingException {
  ...
  @Override
  public Model resolveModel(final ModelBuildingRequest request, final RemoteRepository... repositories) throws ModelBuildingException {
    return resolveModel(request, assertNotNull(sessionProvider, "Repository system session provider not specified").get());
  }
  ...
  private class ModelResolverProxy implements ModelResolver {
    ...
    public ModelSource resolveModel(final String groupId, final String artifactId, final String version) throws UnresolvableModelException {
      try {
        final Artifact pom = artifactResolver.resolveArtifact(request().artifact(groupId, artifactId, version, "pom"), session, remoteRepositories(allRepositories));
