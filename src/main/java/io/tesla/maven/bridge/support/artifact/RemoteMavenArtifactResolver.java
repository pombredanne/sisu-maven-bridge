...
import io.tesla.maven.bridge.Names;
import io.tesla.maven.bridge.internal.RepositorySystemSessionWrapper;
import io.tesla.maven.bridge.support.artifact.internal.MavenArtifactResolverSupport;
...
public class RemoteMavenArtifactResolver extends MavenArtifactResolverSupport implements MavenArtifactResolver {
  ...
  @Inject
  public RemoteMavenArtifactResolver(final ServiceLocator serviceLocator, final @Nullable Provider<RepositorySystemSession> sessionProvider) {
  ...
  protected Artifact doResolve(final ArtifactRequest artifactRequest, final RepositorySystemSession session, final RemoteRepository... repositories) throws ArtifactResolutionException {
    ...
    artifactRequest.setRepositories(remoteRepositoryManager.aggregateRepositories(session, Collections.<RemoteRepository> emptyList(), allRepositories, RECESSIVE_IS_RAW));
    ...
    if (session.getLocalRepositoryManager() == null || session.getLocalRepository() == null) {
      safeSession = new RepositorySystemSessionWrapper(session) {
        final LocalRepositoryManager lrm = repositorySystem.newLocalRepositoryManager(new LocalRepository(new File(Names.MAVEN_USER_HOME, "repository")));
