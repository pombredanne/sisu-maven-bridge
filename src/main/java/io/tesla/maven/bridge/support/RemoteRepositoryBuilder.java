 ...
 */
package io.tesla.maven.bridge.support;
...
public class RemoteRepositoryBuilder {
  ...
  public static RemoteRepository remoteRepository(final String id, final String type, final String url) {
    ...
    repository.setPolicy(false, new RepositoryPolicy(true, RepositoryPolicy.UPDATE_POLICY_DAILY, RepositoryPolicy.CHECKSUM_POLICY_WARN));
    return repository;
  }

  public static RemoteRepository remoteRepository(final Repository repository) {
    final RemoteRepository remoteRepository = new RemoteRepository(repository.getId(), repository.getLayout(), repository.getUrl());
