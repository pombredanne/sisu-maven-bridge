...
import io.tesla.maven.bridge.support.dependency.internal.MavenDependencyTreeResolverSupport;
import io.tesla.maven.bridge.support.model.RemoteMavenModelResolver;
...
public class RemoteMavenDependencyTreeResolver extends MavenDependencyTreeResolverSupport implements MavenDependencyTreeResolver {
  ...
  public RemoteMavenDependencyTreeResolver(final ServiceLocator serviceLocator, final @Nullable RemoteMavenModelResolver mavenModelResolver) {
  ...
  public DependencyNode resolveDependencyTree(final CollectRequest request, final RepositorySystemSession session, final RemoteRepository... repositories) throws DependencyCollectionException {
    ...
    if (session.getLocalRepositoryManager() == null || session.getLocalRepository() == null) {
      safeSession = new RepositorySystemSessionWrapper(session) {
        final LocalRepositoryManager lrm = getRepositorySystem().newLocalRepositoryManager(new LocalRepository(new File(Names.MAVEN_USER_HOME, "repository")));
